...
object TestClassifier {
           ...
           Init_FN:Int, name: String) =
  {
          ...
          for (k1 <- 0 until res2.size()) {
            ...
            for (i1 <- 0 until res.size()) {
              for (j1 <- i1 + 1 until res.size()) {
                val edge: Edge = g.V(res.get(i1).id)
                  .outE().as("ed")
                  .select("ed")
                  .headOption().asInstanceOf[Option[Edge]]
                  .getOrElse(g.V(res.get(j1).id)
                    .outE().as("ed")
                    .select("ed")
   ...
   // } catch {
    ...
    println(s"TN_g = ${TN_g}")
    println(s"FN_g = ${FN_g}")
    val prec_c: Double = TP_c.toDouble / (TP_c + FP_c)
    println(s"precision_c = ${prec_c}")
    val rec_c: Double = TP_c.toDouble / (TP_c + FN_c)
    println(s"recall_c = ${rec_c}")
    val F1_c: Double = 2 * (prec_c * rec_c) / (prec_c + rec_c)
    println(s"F1_c = ${F1_c}")
    val prec_g: Double = TP_g.toDouble / (TP_g + FP_g)
    println(s"precision_g = ${prec_g}")
    val rec_g: Double = TP_g.toDouble / (TP_g + FN_g)
    println(s"recall_g = ${rec_g}")
    val F1_g: Double = 2 * (prec_g * rec_g) / (prec_g + rec_g)
    println(s"F1_g = ${F1_g}")
  }
